...
<div class="min-h-screen flex items-center justify-center p-6 bg-surface">
    <div class="w-full max-w-md bg-surface-container-low border border-outline-variant/30 rounded-3xl p-8 shadow-sm">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold font-display-balance text-on-surface">Mulai MyDuit</h1>
            <p class="text-on-surface-variant text-sm mt-2">Bikin akun buat pantau duit lo.</p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="flex flex-col gap-4">
            ...
            @if ($errors->any())
                <div class="bg-error-container text-on-error-container p-4 rounded-xl text-sm mb-2">
            ...
            <div class="flex flex-col gap-1">
                <label for="name" class="text-sm font-semibold text-on-surface">Nama Panggilan</label>
            ...
            <div class="flex flex-col gap-1">
                ...
                <div class="relative w-full">
                    ...
                    <button type="button" onclick="togglePassword('password', 'eyeIcon1')" class="absolute flex items-center justify-center rounded-full text-on-surface-variant hover:text-primary transition-colors cursor-pointer" style="right: 0.75rem; top: 50%; transform: translateY(-50%); width: 2rem; height: 2rem;">
                        <span id="eyeIcon1" class="material-symbols-outlined" style="font-size: 1.25rem;">visibility</span>
                    </button>
                </div>
            </div>

            <div class="flex flex-col gap-1">
                <label for="password_confirmation" class="text-sm font-semibold text-on-surface">Konfirmasi Password</label>
                <div class="relative w-full">
                    <input type="password" id="password_confirmation" name="password_confirmation" required
                        class="w-full bg-surface text-on-surface border border-outline-variant rounded-xl pl-4 py-3 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors" style="padding-right: 3rem;">
                    <button type="button" onclick="togglePassword('password_confirmation', 'eyeIcon2')" class="absolute flex items-center justify-center rounded-full text-on-surface-variant hover:text-primary transition-colors cursor-pointer" style="right: 0.75rem; top: 50%; transform: translateY(-50%); width: 2rem; height: 2rem;">
                        <span id="eyeIcon2" class="material-symbols-outlined" style="font-size: 1.25rem;">visibility</span>
                    </button>
                </div>
            </div>

            <button type="submit" class="w-full bg-primary text-on-primary py-3 rounded-xl font-semibold hover:bg-primary-fixed-dim transition-colors mt-4 shadow-md">
                Daftar
            </button>
        </form>

        <p class="text-center text-sm text-on-surface-variant mt-8">
            Udah punya akun? <a href="{{ route('login') }}" class="text-primary font-semibold hover:underline">Masuk di sini</a>
        </p>
    </div>
</div>
